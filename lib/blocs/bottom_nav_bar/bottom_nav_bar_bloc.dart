...
class BottomNavBarBloc extends Bloc<BottomNavBarEvent, BottomNavBarState> {
  BottomNavBarBloc() : super(const BottomNavBarVisible('')) {
    on(_onShowNavBar);
    on(_onHiddenNavBar);
  ...
  void _onShowNavBar(ShowBottomNavBar event, Emitter<BottomNavBarState> emit) {
    emit(ChangingBottomNavBarVisible(event.routeName));
    visible = true;
    emit(BottomNavBarVisible(event.routeName));
  }

  void _onHiddenNavBar(HideBottomNavBar event, Emitter<BottomNavBarState> emit) {
    emit(ChangingBottomNavBarVisible(event.routeName));
    visible = false;
  ...
  void _onChangeBottomNavigationTab(
    ...
    emit(BottomNavigationTabChanged(index: event.index, routeName: event.routeName));
  }
}
