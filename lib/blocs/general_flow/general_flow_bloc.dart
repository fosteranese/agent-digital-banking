...
class GeneralFlowBloc extends Bloc<GeneralFlowEvent, GeneralFlowState> {
  ...
  Future<void> _onRetrieveGeneralFlowFormData(
    ...
    try {
      ...
      switch (event.activityType) {
        case ActivityTypesConst.fblOnline:
          stored = await _fblOnlineRepo.getStoredFormData(
            id: event.formId,
            qrCode: event.qrCode,
          ...
          break;
        case ActivityTypesConst.quickFlow:
          stored = await _quickflow.getStoredFormData(
            id: event.formId,
            qrCode: event.qrCode,
            payeeId: event.payeeId,
          );
      ...
      switch (event.activityType) {
        case ActivityTypesConst.fblOnline:
          ...
          break;
        case ActivityTypesConst.quickFlow:
          result = await _quickflow.retrieveFormData(
            id: event.formId,
            qrCode: event.qrCode,
            payeeId: event.payeeId,
          );
  ...
  Future<void> _onVerifyRequest(VerifyRequest event, Emitter<GeneralFlowState> emit) async {
    ...
    try {
      ...
      switch (event.activityType) {
        case ActivityTypesConst.fblOnline:
          stored = await _fblOnlineRepo.verifyRequest(
  ...
  Future<void> _onProcessRequest(ProcessRequest event, Emitter<GeneralFlowState> emit) async {
    try {
      ...
      switch (event.activityType) {
        case ActivityTypesConst.fblOnline:
          stored = await _fblOnlineRepo.processRequest(
    ...
    try {
      ...
      if (stored != null && (stored.data?.sources?.isNotEmpty ?? false)) {
        ...
        emit(
          GeneralFlowEnquired(
            ...
            routeName: event.routeName,
            activityType: event.activityType,
            endpoint: event.form.verifyEndpoint ?? '',
          ),
        );
      ...
      if (stored == null || (stored.data?.sources?.isEmpty ?? false)) {
        ...
        ResponseUtil.handleException(
          ex,
          (error) =>
              emit(SilentEnquireGeneralFlowError(result: error, activityType: event.activityType)),
        );
      }
    }
  }
  ...
  Future<void> _onSubEnquiryGeneralFlow(
    ...
    try {
      ...
      stored = await _fblOnlineRepo.getStoredSubEnquiry(
        endpoint: event.endpoint,
        formId: event.formId,
      ...
      final result = await _fblOnlineRepo.retrieveSubEnquiry(
        endpoint: event.endpoint,
        formId: event.formId,
        hashValue: event.hashValue,
      );
  ...
  Future<void> _onPrepareScheduler(PrepareScheduler event, Emitter<GeneralFlowState> emit) async {
    ...
    try {
      ...
      stored = await _fblOnlineRepo.getStoredPreparedSchedule(
        receiptId: event.receiptId,
        payeeId: event.payeeId,
      );

      final key = (event.receiptId?.isNotEmpty ?? false)
          ? 'r-${event.receiptId}'
          : 'p-${event.payeeId}';
      if (stored != null && (stored.data?.fieldsDatum?.isNotEmpty ?? false)) {
      ...
      final result = await _fblOnlineRepo.prepareScheduler(
        receiptId: event.receiptId,
        payeeId: event.payeeId,
      );
