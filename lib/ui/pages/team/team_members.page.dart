...
import 'package:my_sage_agent/ui/components/headers/team_members_header.dart';
...
class _TeamMembersPageState extends State<TeamMembersPage> {
  ...
  List<Agent>? _sourceList;
  final _list = ValueNotifier<List<Agent>>([]);
  ...
  @override
  void initState() {
    _loadTeamMembers(false);
    _sourceList = context.read<RetrieveDataBloc>().data['RetrieveTeamMembersEvent'];
  ...
  void _loadTeamMembers(bool skipSavedData) {
    context.read<RetrieveDataBloc>().add(
      RetrieveTeamMembersEvent(
        id: Uuid().v4(),
        name: _nameController.text.trim(),
        code: _codeController.text.trim(),
        action: 'RetrieveTeamMembersEvent',
  ...
  Widget build(BuildContext context) {
    return MainLayout(
      showBackBtn: true,
      title: 'Team Members',
      theme: MainLayoutThemes.whiteHeader,
      slivers: [
        ...
        ValueListenableBuilder(
          ...
          builder: (context, list, child) {
            return BlocConsumer<RetrieveDataBloc, RetrieveDataState>(
              ...
              buildWhen: (previous, current) => current.event is RetrieveTeamMembersEvent,
              builder: (context, state) {
                  ...
                  sliver: SliverList.separated(
                    ...
                    itemBuilder: (_, index) {
                      ...
                      return TeamMemberItem(
                        record: record,
                        onTap: () {
                          context.push(AgentDetailsPage.routeName, extra: record);
                        },
                      );
                    },
  ...
  void _handleBlocListener(RetrieveDataState state) {
    if (state.event is! RetrieveTeamMembersEvent) {
      return;
    }
  ...
  void _search(String value, List<Agent> requests) {
    final search = value.trim().toLowerCase();
    _list.value = requests.where((e) {
      return (e.fullName?.toLowerCase().contains(search) ?? false) ||
          (e.agentCode?.toString().toLowerCase().contains(search) ?? false);
    }).toList();
  }
