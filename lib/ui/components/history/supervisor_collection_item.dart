...
class SupervisorCollectionItem extends StatelessWidget {
  final SupervisorCollectionModel record;
  final VoidCallback? onTap;

  const SupervisorCollectionItem({super.key, required this.record, this.onTap});
  ...
  Map<String, dynamic> get _status {
    switch (record.collection?.status) {
      case 1:
        return {
  ...
  Widget _buildDetails() {
    return Expanded(
      child: Column(
        ...
        children: [
          Text(
            record.collection?.customerName ?? 'John Doe',
            maxLines: 2,
  ...
  Widget _buildAmountOrStatus() {
    return Column(
      ...
      children: [
        Text(
          FormatterUtil.currency(record.collection?.amount ?? 0.0),
          style: PrimaryTextStyle(
            color: ThemeUtil.black,
            fontSize: 14,
