...
class MainRemote {
  ...
  Future<Map<String, dynamic>> _formatRequestPayload({
    ...
    if (isAuthenticated) {
      data['meta']['sessionId'] =
          AppUtil.currentUser.sessionId;
    }

    if (encrypt) {
      final jsonText = json.encode(data);
      logger.i(jsonText);
      final (String cipher, Key key, IV iv) =
          await EncryptionUtil.encrypt(jsonText);
  ...
  Future<Response<dynamic>> post({
    ...
    try {
      ...
      final iv = request.containsKey('iv')
          ? (request['iv'] as IV)
          : null;
      final key = request.containsKey('key')
          ? (request['key'] as Key)
          : null;
