...
class ReversalRepo {
  ...
  void storeReversals(List<ReversalRequestModel> list, int? status) {
    _db.add(key: 'reversals-$status', payload: {'list': list});
  }

  Future<List<ReversalRequestModel>?> getStoredReversals({Activity? activity}) async {
    ...
    final list = (result['list'] as List<dynamic>).map((item) {
      if (item is String) {
        return ReversalRequestModel.fromJson(item);
      }

      return ReversalRequestModel.fromMap(item as Map<String, dynamic>);
    }).toList();

    return list;
  }

  Future<List<ReversalRequestModel>> loadReversals({
    int? status,
    DateTime? startDate,
    DateTime? endDate,
  }) async {
  ...
  Future<List<ReversalRequestModel>> loadSupervisorAgentReversals({
  ...
  Future<List<ReversalRequestModel>> loadSupervisorReversals({
    DateTime? startDate,
    DateTime? endDate,
  }) async {
    final sDate = startDate?.toIso8601String();
  ...
  Future<List<ReversalRequestModel>> loadAgentReversals({
    DateTime? startDate,
    DateTime? endDate,
  }) async {
    final sDate = startDate?.toIso8601String();
    final eDate = endDate?.toIso8601String();
    final response = await _fbl.post(
      path: 'FieldExecutive/getAgentReversals',
      body: {"startDate": sDate, "endDate": eDate},
      isAuthenticated: true,
    );
