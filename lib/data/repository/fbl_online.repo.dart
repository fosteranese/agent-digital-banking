...
class FblOnlineRepo {
  ...
  Future<Response<Enquiry>?> getStoredSubEnquiry({
  ...
  Future<Response<Enquiry>> retrieveSubEnquiry({
    required String endpoint,
    required String formId,
    required String hashValue,
  }) async {
    final response = await _fbl.post(
      path: endpoint,
      body: {'formId': formId, 'hashValue': hashValue},
      isAuthenticated: true,
    );
  ...
  Future<Response<GeneralFlowFormData>> prepareScheduler({
    ...
    final response = await _fbl.post(
      path: 'Schedule/requestSchedule',
      body: {'receiptId': receiptId, 'payeeId': payeeId},
      isAuthenticated: true,
    );
